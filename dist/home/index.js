...
(() => {
  ...
  // src/home/utils/gsap.ts
  gsapWithCSS.registerPlugin(ScrollTrigger2);
  function startSequentielleAnimation() {
    const tl = gsapWithCSS.timeline();
    tl.fromTo(
      "#hero-heading",
      {
        x: "-100%",
        // Startet von 100% links außerhalb des Bildschirms
        opacity: 0
        // Startet mit einer Opazität von 0
      },
      {
        x: "0%",
        // Endposition (normalerweise ist das die Standardposition)
        opacity: 1,
        // Endet mit einer Opazität von 1
        duration: 0.75
      }
    ).fromTo(
      "#sub-hero",
      {
        y: "20%",
        // Startet leicht nach oben verschoben
        opacity: 0
        // Startet mit einer Opazität von 0
      },
      {
        y: "0%",
        // Bewegt sich zurück in die normale Position
        opacity: 1,
        // Endet mit einer Opazität von 1
        duration: 0.75,
        ease: "power1.out"
      ...
      "-=0.25"
      // Fügt eine Verzögerung nach der vorherigen Animation hinzu
    ).to("#breadcrumb-component", { duration: 1, opacity: 1 }, "-=0.3").to("#navbar-component", { duration: 1, opacity: 1 }, "<").to("#hero-video", { duration: 3, opacity: 1, ease: "power2.out" }, "-=0.3");
  }
  function siegelStagger() {
    gsapWithCSS.to(".logo2_logo", {
      scrollTrigger: {
        trigger: "#section_logo2",
        // Element, das die Animation auslöst
        start: "top center",
        // Startet die Animation, wenn der oberen Rand von #section_logo2 den Mittelpunkt des Viewports erreicht
        end: "bottom center",
        // Optional: Definiert das Ende der ScrollTrigger-Überwachung
        toggleActions: "play none none none"
        // Definiert, was passiert, wenn die ScrollTrigger-Bedingungen erfüllt sind
      ...
      // Art des Easings
      stagger: 0.1
      // Verzögerung zwischen den Animationen jedes Links
    });
  }
  function carousel() {
    gsapWithCSS.fromTo(
      "#modularplattform-component",
      {
        ...
        // Startet mit einer Opazität von 0
        y: 20
        // Startet leicht nach unten verschoben (optional, falls gewünscht)
      },
      {
        scrollTrigger: {
          trigger: "#section_modularplattform",
          // Element, das die Animation auslöst
          start: "top center"
          // Startet die Animation, wenn der obere Rand von #section_modularplattform den Mittelpunkt des Viewports erreicht
        },
        y: 0,
        // Bewegt sich zurück in die normale Position
        opacity: 1,
  ...
  function timeline2() {
    gsapWithCSS.to("#timeline-item", {
      scrollTrigger: {
        trigger: "#section_stats52",
        // Das Element, das den ScrollTrigger auslöst
        start: "top center",
        // Beginnt die Animation, wenn der obere Rand von #section_stats52 den Mittelpunkt des Viewports erreicht
        end: "bottom center",
        // Optional: Definiert das Ende der ScrollTrigger-Überwachung
        toggleActions: "play none none none"
        // Definiert, was passiert, wenn die ScrollTrigger-Bedingungen erfüllt sind
      ...
      ease: "power1.inOut",
      stagger: 0.2
      // Verzögerung zwischen den Animationen jedes Elements
    });
  }
  function sustainableSection() {
    const tl = gsapWithCSS.timeline({
      scrollTrigger: {
        trigger: "#section_sustainable",
        // Das Element, das den ScrollTrigger auslöst
        start: "top center",
        // Startet die Animation, wenn der obere Rand von #section_sustainable den Mittelpunkt des Viewports erreicht
        end: "bottom center",
        // Optional: Definiert das Ende der ScrollTrigger-Überwachung
        toggleActions: "play none none none"
        // Definiert, was passiert, wenn die ScrollTrigger-Bedingungen erfüllt sind
    ...
    tl.from("#sustainable-component_upper-row", {
      ...
      "#sustainable-component_upper-row",
      {
        ...
        y: 0,
        ease: "power1.out"
        // Tippfehler korrigiert von 'power2.oupt' zu 'power2.out'
      },
      "-=0.25"
    ).to(
      "#sustainable-component_lower-row",
  ...
  // src/home/utils/Mapbox.ts
  var initializeMapbox = () => {
    mapboxgl.accessToken = "pk.eyJ1IjoiamFrb2J2YXIiLCJhIjoiY2xyeXlxa2w5MWtxYzJqbmFmZXJ4MXRhZiJ9.TxpTMaCXvDgq5akCtF1kZQ";
    ...
    locations.forEach(function(loc) {
      ...
      const popup = new mapboxgl.Popup({ offset: 25 }).setHTML(`
        ...
        `);
      new mapboxgl.Marker(el, { anchor: "bottom" }).setLngLat(loc.coordinates).setPopup(popup).addTo(map);
    });
    map.on("load", function() {
      const bounds = locations.reduce(
        function(bounds2, loc) {
          return bounds2.extend(loc.coordinates);
        },
        new mapboxgl.LngLatBounds(locations[0].coordinates, locations[0].coordinates)
      );
      map.fitBounds(bounds, {
        padding: 50,
        // Optionaler Abstand von den Rändern der Karte in Pixeln
        maxZoom: isMobileDevice() ? 2 : 3
